...
namespace CarManagement.Controllers
{
    public class ElectricCarsController : Controller
    {
        ...
        public ElectricCarsController(IConfiguration configuration)
        {
        ...
        public IActionResult Index()
        {
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM ElectricCar", conn);
        ...
        public IActionResult Edit(int id)
        {
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM ElectricCar WHERE id = @id", conn);
        ...
        [HttpPost]
        public IActionResult Edit(ElectricCar car)
        {
            using (var conn = new MySqlConnection(GetConnection()))
            {
                conn.Open();
                var cmd = new MySqlCommand(@"
                    UPDATE ElectricCar 
                    SET battery = @battery, range_km = @range, model = @model, 
                        branch_id = @branch, price = @price, imageUrl = @url 
                    WHERE id = @id", conn);
                cmd.Parameters.AddWithValue("@battery", car.Battery);
                cmd.Parameters.AddWithValue("@range", car.RangeKm);
        ...
        public IActionResult DeleteConfirmed(int id)
        {
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("DELETE FROM ElectricCar WHERE id = @id", conn);
                cmd.Parameters.AddWithValue("@id", id);
                cmd.ExecuteNonQuery();
            }
            return RedirectToAction("Index");
